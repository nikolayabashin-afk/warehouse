...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  return <html lang="ru"><body>
    <div className="min-h-screen flex bg-gray-50">
      {isLoggedIn && <aside className="w-64 bg-gray-950 text-white p-5 hidden md:block overflow-y-auto">
      ...
      <div className="flex-1 min-w-0">
        {isLoggedIn && <header className="sticky top-0 z-10 border-b bg-white/90 backdrop-blur px-4 py-3 md:px-8 md:py-4">
        ...
        <main className={isLoggedIn ? "p-4 md:p-8" : "p-4 md:p-8"}>{children}</main>
      </div>
    </div>
  </body></html>
}
